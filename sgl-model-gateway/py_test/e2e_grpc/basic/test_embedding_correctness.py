...
class TestEmbeddingCorrectness(CustomTestCase):
    ...
    Strategy: Pre-compute HuggingFace reference embeddings BEFORE launching the worker
    to avoid GPU memory conflicts. Then launch the worker and compare.
    """
    ...
    def setUpClass(cls):
        ...
        # Pre-compute all reference embeddings BEFORE launching the worker
        # This allows us to use GPU for HuggingFace, then free memory for the worker
        logging.info(f"Pre-computing HuggingFace reference embeddings for {cls.model}")
        ...
        logging.info("Reference embeddings computed, freeing GPU memory...")
